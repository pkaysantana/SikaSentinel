...
async function main(): Promise<void> {
  ...
  stageLabel("Context Engine");
  printRuntimeContext(r1.runtimeContext!);

  stageLabel("Clearing Agent");
  field("policy decision", r1.decision?.allowed ? "✔  ALLOWED" : "✘  DENIED");
  field("rule",            r1.decision?.ruleId ?? "—");

  stageLabel("Execution Adapter");
  const balance = await getBalance(OPERATOR_ID);
  field("balance",         `${balance.hbar.toFixed(8)} HBAR`);
  field("mode",            balance.stub ? "stub (in-memory)" : "live (Hedera)");

  stageLabel("Evidence Layer");
  field("audit id",        `${r1.auditEntry!.auditId.slice(0, 8)}…`);
  field("topic",           r1.auditEntry!.topicId ?? "—");
  ...
  stageLabel("Context Engine");
  printRuntimeContext(r2.runtimeContext!);
  ...
  field("topic",            r2.auditEntry!.topicId ?? "—");
  ...
  stageLabel("Context Engine");
  printRuntimeContext(r3.runtimeContext!);
  ...
  field("topic",            r3.auditEntry!.topicId ?? "—");
